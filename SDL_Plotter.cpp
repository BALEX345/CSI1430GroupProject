...
// ----- NEW FUNCTION ----- //
void SDL_Plotter::plotSquare(point f, color c, int s){
    for (int i = 0; i < s; i++)
    {
        for (int j = 0; j < s; j++)
        {
            plotPixel(f.x + j, f.y + i, c.R, c.G, c.B);
        }
    }
...
// ----- NEW FUNCTION ----- //
void SDL_Plotter::backgroundColor(int c){
    if(c > 255 || c < 0){
        c = 0;
    }
    memset(pixels, c, col * row * sizeof(Uint32));
}
